<script lang="ts">
  ...
  const recentComments = [
    ...
    },
    {
      ...
      author: "Anna Schmidt",
      content:
        "Kann ich nur bestätigen, diese Methoden funktionieren wirklich.",
      date: "2024-01-14",
      isApproved: false,
    },
  ];
  ...
  <div class="mb-8">
    <h1 class="text-4xl font-bold text-gray-900 mb-2">
      Willkommen zurück, <span class="text-purple-600"
        >{$userStore.display_name}</span
      >!
    </h1>
    <p class="text-gray-600">
      Organisiere dich und behalte dein Leben auf Band.
    </p>
  </div>
        ...
        {#each recentComments as comment}
          <div class="border border-gray-200 rounded-lg p-4">
            <div class="flex items-start justify-between mb-2">
              ...
              <span
                class="px-2 py-1 text-xs rounded-full {comment.isApproved
                  ? 'bg-green-100 text-green-800'
                  : 'bg-yellow-100 text-yellow-800'}"
              >
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        <button
          ...
          on:click={() => (window.location.href = "/entry")}
        >
          <svg
