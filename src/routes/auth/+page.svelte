<script lang="ts">
	...
	function validateForm(): string | null {
		...
		if (mode === 'signup') {
			if (nickname.length < 3) return 'Der Nickname muss mindestens 3 Zeichen haben.';
            if (nickname.length > 20) return 'Da hast du dich schon durchgemogelt und darst dennoch kein längeren Nickname haben.';
			if (password !== passwordConfirm) return 'Passwörter stimmen nicht überein.';
		}
		return null;
	}

	function mapAuthError(error: any): string { // Error Handling, um auch in einer Sprache zu bleiben
...
<section class="max-w-md mx-auto my-10 p-6 rounded-xl backdrop-blur border border-purple bg-nav">
	...
	<form on:submit|preventDefault={handleAuth} class="space-y-4">
		...
		<div>
			<label for="password" class="block text-sm text-white mb-1">Passwort</label>
			<input
				id="password"
				type="password"
				bind:value={password}
		...
		{#if mode === 'signup'}
			<div>
				<label for="nickname" class="block text-sm text-white mb-1">Username</label>
				<input
					id="nickname"
					type="text"
                    maxlength="20"
